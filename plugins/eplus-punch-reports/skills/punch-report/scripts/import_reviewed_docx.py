...
STATUS: PROTOTYPE. NOT YET RUN END TO END AS A SINGLE PROGRAM. Assembled from
the ad hoc code that recovered 16 approved write-ups from a reviewed report on
one project and mapped every one to its PlanGrid pin. Packaged here as the
starting point for a first-class script. Expect to debug it the first time it
runs; check its output by hand before trusting the drafted entries it writes.
...
discards the reviewer's approved text. On the project this was written for the
reviewer had also deleted two items, merged two pins into one, retyped several
headings (destroying their bookmarks) and silently dropped one photo. None of
that is recorded anywhere except in the document itself.
...
was safe on this project. build_master.py applies the same check again to any
entry whose origin is reviewer_final or user_reviewed.

Items the reviewer merged (one report item matching photos from two pins) are
SKIPPED here. Encode them as a "merges" block in drafted_items.json
({"items": [...], "merges": [{"into": N, "from": M, "drop_photos": [...]}]}),
which build_master.py applies in memory; never mutate items.json.
...
# ------------------------------------------------------------- pixel matching
def signature(path):
    """32x32 greyscale pixel vector, scaled by its own std so exposure differences
    do not dominate. Pure Python on purpose: numpy is not a pipeline dependency."""
    from PIL import Image
    im = Image.open(path).convert('L').resize((32, 32))
    vals = [float(v) for v in im.getdata()]
    mean = sum(vals) / len(vals)
    std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5 or 1.0
    return [v / std for v in vals]


def distance(a, b):
    """Mean squared difference between two signatures."""
...
def main():
    ap = argparse.ArgumentParser(
        description='PROTOTYPE, not yet run end to end. Recover reviewer-approved '
                    'wording from an edited report into drafted_items.json.')
    ...
    for rec in reviewed:
        ...
        if len(rec['pins']) > 1:
            problems.append(f"item {rec['item_number']}: spans pins {rec['pins']}, "
                            f"the reviewer merged them. Encode as a merges block "
                            f"in drafted_items.json; the absorbed pin is omitted "
                            f"automatically.")
        if not rec['pins']:
    ...
    json.dump(drafted, open(args.out, 'w', encoding='utf-8'), indent=2)
    print(f'\nwrote {len(drafted)} reviewer_final items -> {args.out}')
    print('Items spanning multiple pins were SKIPPED and must be encoded as a '
          'merges block in drafted_items.json before re-running.')
    print('PROTOTYPE: check every recovered entry against the reviewed document '
