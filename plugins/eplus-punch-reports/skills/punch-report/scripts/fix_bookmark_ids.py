...
STATUS: shipped. First used on a revision of a live report after the bookmark
defect below reached a reviewer.
...
shipped in one revision and is exactly the class of defect verify_report.py
exists to catch, so the matching assertion was added there too.
...
relied on rather than correctness. Rewritten to the form Word writes.

Runs after every render (wired into run_pipeline.sh). Edits the file in place.
