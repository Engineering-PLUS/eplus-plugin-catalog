...
- Read the `brand-summary.md` file located in this skill's directory to load the exact color palette (Blue: #666f89, Green: #3c7d7f, etc.) and the default-font typography hierarchy.
...
- Set the `FontFamily` property to `Arial` (e.g., `FontFamily="Arial"`). Use `FontWeight="Bold"` for headings/sub-headings; headlines may use `Arial Black` where available.
...
- Use `Arial Black` (or `Arial Bold`) for headlines, `Arial Bold` for sub-headings and section headings, and `Arial Regular` for body copy.
...
- **Colors:** Strictly use the HEX colors provided in `brand-summary.md`. Never hallucinate brand colors.
...
- **Typography:** Apply the brand hierarchy using default fonts only — `Arial Black` for headlines, `Arial Bold` for sub-headlines and section headings (Title Case or ALL CAPS, not exceeding headline size), and `Arial Regular` for body copy (sentence case). Never declare, embed, or reference `Montserrat` or `Sui Generis` in this edition.
